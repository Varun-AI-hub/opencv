...
// Regression test for https://github.com/opencv/opencv/issues/19366
// Verifies that ONNX models with BOOL constant tensors (stored as raw_data or int32_data)
// load without throwing "Unsupported data type: BOOL".
TEST(Test_ONNX_importer, BoolTensorRawData)
{
    // Minimal ONNX ModelProto: a single Constant node whose attribute tensor has
    // data_type=BOOL (9) and raw_data = {0x01, 0x00, 0x01} (3 bool values).
    // Pre-encoded as a protobuf byte array so no external .onnx file is required.
    static const unsigned char kModel[] = {
        0x08, 0x06, 0x3a, 0x41, 0x0a, 0x29, 0x12, 0x03,
        0x6f, 0x75, 0x74, 0x22, 0x08, 0x43, 0x6f, 0x6e,
        0x73, 0x74, 0x61, 0x6e, 0x74, 0x2a, 0x18, 0x0a,
        0x05, 0x76, 0x61, 0x6c, 0x75, 0x65, 0x20, 0x04,
        0x32, 0x0d, 0x08, 0x03, 0x10, 0x09, 0x42, 0x02,
        0x62, 0x63, 0x4a, 0x03, 0x01, 0x00, 0x01, 0x12,
        0x01, 0x67, 0x62, 0x11, 0x0a, 0x03, 0x6f, 0x75,
        0x74, 0x12, 0x0a, 0x0a, 0x08, 0x08, 0x09, 0x12,
        0x04, 0x0a, 0x02, 0x08, 0x03, 0x42, 0x02, 0x10, 0x09
    };
    std::vector<uchar> buf(kModel, kModel + sizeof(kModel));
    Net net;
    ASSERT_NO_THROW(net = readNetFromONNX(buf));
    EXPECT_FALSE(net.empty());
}

TEST(Test_ONNX_importer, BoolTensorInt32Data)
{
    // Same as BoolTensorRawData but the tensor uses int32_data (packed field 5)
    // instead of raw_data.  This is the other encoding allowed by the ONNX spec
    // and was the one that previously triggered the "Unsupported data type: BOOL" error.
    static const unsigned char kModel[] = {
        0x08, 0x06, 0x3a, 0x43, 0x0a, 0x2a, 0x12, 0x04,
        0x6f, 0x75, 0x74, 0x69, 0x22, 0x08, 0x43, 0x6f,
        0x6e, 0x73, 0x74, 0x61, 0x6e, 0x74, 0x2a, 0x18,
        0x0a, 0x05, 0x76, 0x61, 0x6c, 0x75, 0x65, 0x20,
        0x04, 0x32, 0x0d, 0x08, 0x03, 0x10, 0x09, 0x42,
        0x02, 0x62, 0x69, 0x2a, 0x03, 0x01, 0x00, 0x01,
        0x12, 0x01, 0x67, 0x62, 0x12, 0x0a, 0x04, 0x6f,
        0x75, 0x74, 0x69, 0x12, 0x0a, 0x0a, 0x08, 0x08,
        0x09, 0x12, 0x04, 0x0a, 0x02, 0x08, 0x03, 0x42,
        0x02, 0x10, 0x09
    };
    std::vector<uchar> buf(kModel, kModel + sizeof(kModel));
    Net net;
